...
# Two countries (West and East) on the same lattice. The counterfactual is a
# move from autarky between the countries to free trade, holding the transport
# network fixed, so dist0 and dist1 coincide here and only the trade weights
# (dclosed vs dopen) change. Ported from
# GTFM_MatlabPrograms/countries/imperfmobdata.m; the figures are omitted.

# Run from this folder:  julia imperfmobdata.jl
...
realwge = realwage(param, fund, L, w, tradesh, dist0, nobs)
...
include("functions/solveabCtyClosed.jl")
a_i, b_i, abtradesh, aconverge, bconverge, xtic =
    solveabCtyClosed(param, observe, dclosed, dist0, nobs)
...
include("functions/solveLwCtyOpen.jl")
Cw, CL, Ctradesh, Cdtradesh, CLconverge, Cwconverge, Cxtic =
    solveLwCtyOpen(param, fund, dopen, dist1, nobs)
...
Crealwge = realwage(param, fund, CL, Cw, Ctradesh, dist1, nobs)
...
include("functions/obwelfaregains.jl")
...
include("functions/solveabCtyOpen.jl")
Ca_i, Cb_i, Cabtradesh, Caconverge, Cbconverge, Cxtic =
    solveabCtyOpen(param, Cobserve, dopen, dist1, nobs)
...
# Welfare gains
WELFGAIN = welfaregains(param, CTRADESH, TRADESH, COBSERVE[:, 1], OBSERVE[:, 1], NOBSC)
...
# Perfectly mobile welfare gains
MOBWELFGAIN = mobwelfaregains(param, CTRADESH, TRADESH, COBSERVE[:, 1], OBSERVE[:, 1], NOBSC)
